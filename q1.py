...
def build_prompt(notes_json_str: str) -> str:
    ...
    n_scores = max(len(notes) - 1, 0)

    return f"""You are a senior speech-language pathologist (SLP) with extensive experience \
evaluating paediatric therapy records. Your task is to review a child's ordered session notes \
and assess how much progress the child made between each consecutive pair of sessions.

## Rating scale

For each transition from Session N to Session N+1, assign ONE integer score:

| Score | Label                     | When to use |
|-------|---------------------------|-------------|
| 0     | Maintenance / minimal change | The child is functioning at essentially the same level. Accuracy, cueing needs, and hierarchy level remain similar. No meaningful difference between the two notes. |
| 1     | Small but clear improvement  | Modest progress within the same general level — slightly better consistency, less cueing, or improved carryover — but NO major jump in independence or hierarchy level. |
| 2     | Meaningful clinical progress | An obvious step forward that matters clinically: moving from inconsistent to fairly consistent performance, clearly requiring less support, or showing broader generalisation across activities. |
| 3     | Major gain / step up in level | A clear breakthrough: a hierarchy jump, a major gain in independence, or a new level of spontaneous use. Use sparingly — this score is reserved for unmistakable leaps. |

## Boundary clarifications

- Score 0 vs 1: If the difference is only noise or a single data point, use 0. Score 1 requires a clear, replicable improvement that any SLP would notice.
- Score 1 vs 2: Score 1 stays within the same performance band (e.g., 60% → 70% accuracy with same cueing). Score 2 crosses a clinically meaningful threshold (e.g., high cueing → minimal cueing, or inconsistent → consistent).
- Score 2 vs 3: Score 3 is a hierarchy jump or sudden emergence of spontaneous use. If the child improved strongly but is still at the same goal level, use 2.

## Task

Below are {len(notes)} ordered session notes for one child. \
Read them carefully in sequence, then score each consecutive pair.

You must return exactly {n_scores} scores (one per transition: \
{", ".join(f"Session {i}→{i+1}" for i in range(1, len(notes)))}).
...
Return ONLY a JSON array of {n_scores} integers, each between 0 and 3. \
No explanation, no markdown, no extra keys. Example format:
[1, 2, 0, 3, 1, 2, 2, 1, 0, 2, 1]"""
...
def compute_metrics(step_rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if n == 0:
        return {"exact_accuracy": 0.0, "mae": 0.0, "quadratic_weighted_kappa": 0.0}

    # Exact accuracy
    exact_accuracy = sum(t == p for t, p in zip(true_scores, pred_scores)) / n

    # Mean absolute error (interpretable on the 0–3 scale)
    mae = sum(abs(t - p) for t, p in zip(true_scores, pred_scores)) / n

    # Quadratic Weighted Kappa (QWK)
    # Best metric for ordinal agreement: penalises large errors more than small
    # ones, matching the clinical reality that a 0-vs-3 mistake is far worse
    # than a 0-vs-1 mistake.
    labels = list(range(4))  # 0, 1, 2, 3
    k = len(labels)
    label_index = {v: i for i, v in enumerate(labels)}

    # Observed agreement matrix O[i][j] = count(true=i, pred=j)
    O = [[0] * k for _ in range(k)]
    for t, p in zip(true_scores, pred_scores):
        if t in label_index and p in label_index:
            O[label_index[t]][label_index[p]] += 1
    ...
    # Row and column histograms for expected agreement
    row_hist = [sum(O[i]) for i in range(k)]
    col_hist = [sum(O[i][j] for i in range(k)) for j in range(k)]

    # Expected agreement matrix E[i][j] = row_hist[i] * col_hist[j] / n
    E = [
        [(row_hist[i] * col_hist[j]) / n for j in range(k)]
        for i in range(k)
    ]
...
if __name__ == "__main__":
    LABELED_CONFIG = Q1ALabeledConfig(
        test_path="/Users/aashrita/assignment3/labeled_notes.json",
        evaluated_output_path="/Users/aashrita/assignment3/output/q1/evaluated_labeled_results.json",
    )
    UNLABELED_CONFIG = Q1BUnlabeledConfig(
        unlabeled_path="/Users/aashrita/assignment3/unlabeled_notes.json",
        output_path="/Users/aashrita/assignment3/output/q1/scored_notes.json",
    )
